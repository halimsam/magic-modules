...
* `data_cache_count` (Optional) - Number of raw-block local NVMe SSD disks to be attached to the node utilized for GKE Data Cache. Each local SSD is 375 GB in size. If zero, then GKE Data Cache will not be
enabled in the nodes.
